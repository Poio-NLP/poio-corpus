...
def main(argv):
    ...
    for iso_639_3 in config.options("LanguagesISOMap"):
        ...
        corpus_archives = glob.glob(
            os.path.join(corpus_dir, "{0}*.zip".format(iso_639_3))
        )
        ...
        for f in corpus_archives:
            ...
            try:
                ...
                text_files = glob.glob(
                    os.path.join(tmp_path, "{0}*.txt".format(filebasename))
                )
                ...
                if (
                    not arg_iso
                    and iso_639_3 in processed["prediction"]
                    and int(processed["prediction"][iso_639_3]) >= int(wiki_date)
                ):
                ...
                for ngram_size in [1, 2, 3]:
                    print (
                        "  Parsing {0} for cardinality {1}...".format(
